...
class GAN_interpolation_Generator(torch.nn.Module): #input is N X C X H X W x 2 
	def __init__(self,,N,conv_layers_size):
		super(GAN_interpolation_Generator,self).__init__():
		self.conv_first_1 = nn.conv2d(N,16,kernel_size=3,stride=1,padding=1)
		self.conv_final_1 = nn.conv2d(N,16,kernel_size=3,stride=1,padding=1)
		self.conv_first_list = nn.ModuleList([nn.conv2d(16,16,kernel_size = 5,stride=1,padding=2) for i in range(conv_layers_size-1)])
		self.conv_final_list = nn.ModuleList([nn.conv2d(16,16,kernel_size = 5,stride=1,padding=2) for i in range(conv_layers_size-1)])
	...
	def forward(self,x):
		...
		for i in range(len(conv_first_list)):
		...
		output = self.final_layer(x_first,x_final)
		return torch.squeeze(output)


class GAN_interpolation_Discriminator(torch.nn.Module): #input is N X C X H X W x 2 
	def __init__(self,N,flattened_img_size,hidden_size):
		super(GAN_interpolation_Generator,self,).__init__():
		self.conv1 = nn.conv2d(N,16,kernel_size=3,stride=1,padding=1)
		self.conv2 = nn.conv2d(16,16,kernel_size=3,stride=1,padding=1)
		self.linear1 = nn.Linear(flattened_img_size,hidden_size)
